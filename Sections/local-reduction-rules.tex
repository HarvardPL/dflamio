...
{\begin{matrix} \multirow{2}{*}{$\level' = \bigjoin_{\mathclap{\mathsf{s} \in \env(n).\strategy}} \mathsf{s}$} & \quad \forall \level~.~\notactsforquery{n; \env}{p}{q}{\level} \\ & \quad \nopostflowstoquery{n; \env}{\env(n).\lblkw \join \level'}{\conf{n}}\end{matrix}} \\\\
