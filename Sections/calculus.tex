...
Given a global typing environment $\tenv$ we write $\wf{m; \tenv}{\store}$ if, for all $\addr$ such that $\store(\addr) = \lb{p}{\expr}$ and $\tenv_m(\addr) = \reftype[n]{\type}$ it holds that $\hastype{n; \tenv}{\expr}{\type}$. We write $\hastype{n; \tenv}{\config{\store}{\expr}}{\type}$ if $\wf{n; \tenv}{\store}$ and $\hastype{n; \tenv}{\expr}{\type}$. We lift this definition to global configurations and write $\hastype[m]{\tenv}{\gconfig{n}{\env}{S}}{\type}$ if for all $n' \in \Nameset$ there exists a type $\type'$ such that $\hastype{n'; \tenv}{S_{n'}}{\type'}$, and furthermore, when $n' = m$ we have $\type' = \type$.
