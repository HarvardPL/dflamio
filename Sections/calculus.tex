...
\paragraph{Querying trust relationships}
...
\begin{figure}
    \centering
    \begin{mathpar}
    \inferrule[Bot]{}{\actsforquery{\Assumps; n; \env}{p}{\bot}{\bot^{\flowsto}}}
    \and
    \inferrule[Top]{}{\actsforquery{\Assumps; n; \env}{\top}{p}{\bot^{\flowsto}}}
    \and
    \inferrule[Refl]{}{\actsforquery{\Assumps; n; \env}{p}{p}{\bot^{\flowsto}}}
    \and
    \inferrule[Hyp]{(p \actsfor q) \in \Assumps}{\actsforquery{\Assumps; n; \env}{p}{q}{\bot^{\flowsto}}}
    \and
    \inferrule[Proj]{\actsforquery{\Assumps; n; \env}{p}{q}{\level}}{\actsforquery{\Assumps; n; \env}{\authproj{p}}{\authproj{q}}{\level}}
    \and
    \inferrule[ProjR]{}{\actsforquery{\Assumps; n; \env}{p}{\authproj{p}}{\bot^{\flowsto}}}
    \and
    \inferrule[Own-1]{\actsforquery{\Assumps; n; \env}{o}{o'}{\level_1} \\\\ \actsforquery{\Assumps; n; \env}{p}{p'}{\level_2} \\\\ \level = \level_1 \join \level_2}{\actsforquery{\Assumps; n; \env}{\owner{o}{p}}{\owner{o'}{p'}}{\level}}
    \and
    \inferrule[Own-2]{\actsforquery{\Assumps; n; \env}{o}{o'}{\level_1} \\\\ \actsforquery{\Assumps; n; \env}{p}{\owner{o'}{p'}}{\level_2} \\\\ \level = \level_1 \join \level_2}{\actsforquery{\Assumps; n; \env}{\owner{o}{p}}{\owner{o'}{p'}}{\level}}
    \and
    \inferrule[Conj-L]{j \in \{ 1, 2 \} \\\\ \actsforquery{\Assumps; n; \env}{p_j}{p}{\level}}{\actsforquery{\Assumps; n; \env}{p_1 \conj p_2}{p}{\level}}
    \and
    \inferrule[Conj-R]{\actsforquery{\Assumps; n; \env}{p}{p_1}{\level_1} \\\\\ \actsforquery{\Assumps; n; \env}{p}{p_2}{\level_2} \\\\ \level = \level_1 \join \level_2}{\actsforquery{\Assumps; n; \env}{p}{p_1 \conj p_2}{\level}}
    \and
    \inferrule[Disj-L]{\actsforquery{\Assumps; n; \env}{p_1}{p}{\level_1} \\\\\ \actsforquery{\Assumps; n; \env}{p_2}{p}{\level_2} \\\\ \level = \level_1 \join \level_2}{\actsforquery{\Assumps; n; \env}{p_1 \disj p_2}{p}{\level}}
    \and
    \inferrule[Disj-R]{j \in \{ 1, 2 \} \\\\ \actsforquery{\Assumps; n; \env}{p}{p_j}{\level}}{\actsforquery{\Assumps; n; \env}{p}{p_1 \disj p_2}{\level}}
    \and
    \inferrule[Trans]{\actsforquery{\Assumps; n; \env}{p}{q}{\level_1} \\\\ \actsforquery{\Assumps; n; \env}{q}{r}{\level_2}}{\actsforquery{\Assumps; n; \env}{p}{r}{\level_1 \join \level_2}}
    \and
    \inferrule[Del]{\mathsf{ss} = \env_n . \strategy \\\\ \actsforquerysmany{\Assumps; n; \env}{\mathsf{ss}}{p}{q}{\level}}{\actsforquery{\Assumps; n; \env}{p}{q}{\level}}
    \and
    \inferrule[Fwd]{\flowstoquery{\Assumps; n; \env}{\env_n . \lblkw}{m}{\level_1} \\\\ \actsforquery{\Assumps; m; \extend{\env}{m}{\extend{\env_m}{\strategy}{\env_n.\strategy}}}{p}{q}{\level_2} }{\actsforquery{\Assumps; n; \env}{p}{q}{\level_1 \join \level_2}}
    \end{mathpar}
    \caption{Acts for judgment of \lang.}
    \label{fig:act-for-judgment}
\end{figure}

\TODO{We should have an example demonstrating the usage of fluid scopes for delegations and strategies for controlling the raising of the current label because of trust checking.}
...
Note that the computation is decentralized and multiple machines with different resources stored in their memories can cooperate and invoke each other. But the semantics is still deterministic in that only one expression will be reducible at any point. This excludes many possible attacks normally found in distributed information-flow systems \cite{Cite attacks on concurrent and distributed systems} \TODO{Is this claim too bold?}, while still allowing multiple nodes to share computation.
...
Many rules translate directly from \cite{Arden:2015:FA:2859845.2859998}, but differ in how delegations are handled and how remote nodes are used to derive trust relationships. We write $\bot^{\flowsto}$ for the bottom element of the lattice $(\mathcal{P}, \flowsto)$, which correspond to the least confidential and most trusted principal in the lattice. Rule \ruleref{Bot} says that the element $\bot$ trusts any principal, and that deriving this trust requires information up to level $\bot^{\flowsto}$. Dually, \ruleref{Top} states that any principal trusts $\top$, and \ruleref{Refl} states that any element trusts itself. Rule \ruleref{Hyp} states that any hypothesis can be used to derive trust without raising the current label any further. We will see that this is secure since the only way for a trust relationship $p \actsfor q$ to become a hypothesis in $\Assumps$ is to check that this relation can be derived from the rules already. \TODO{Revisit this explanation. Possible make the relationship between this and checked endorsements explicit.} Rule \ruleref{Proj} expresses applying projections preserve trust between principals and \ruleref{ProjR} states that a principal is at least as trusted as the projected principal. Rules \ruleref{Own-1} and \ruleref{Own-2} derives trust between ownership projections. First, \ruleref{Own-1} shows that trust between principals imply trust between owned principals, and \ruleref{Own-2} states that, if an ownership projection $\owner{o'}{p'}$ trusts a principal $p$ then another ownership principal $\owner{o}{p}$ also trusts $\owner{o'}{p'}$, but only if the the owner $o'$ trusts $o$. Rules \ruleref{Conj-L}, \ruleref{Conj-R}, \ruleref{Disj-L} and \ruleref{Disj-R} correspond to the usual lattice rules for joins and meets, and \ruleref{Trans} ensures that the trust relation is transitive. Rule \ruleref{Fwd} expresses how a node $n$ can query another node $m$ for a trust relationship, but only if $n$ allows the information that caused $n$ to contact $m$ to be learned by $m$. Furthermore, the searching strategy used by $n$ should also be used by $m$.

Finally, rule \ruleref{Del} expresses how delegations can be used to derive trust. The judgment uses the search strategy to control how the information-flow lattice should be searched. This is shown in Figure~\ref{fig:act-for-judgment-del}, where the first element in $s$ in the strategy that successfully derives the trust relationship terminates the proof search. Also note that this is deterministic in the sense that the principals in the strategy is searched from beginning to end until one succeeds. The rules \ruleref{Del-1} and \ruleref{Del-2} uses a an interpretation function $\llbracket \cdot \rrbracket(\Assumps, n, \env)$ to compute the set of usable delegations for a node $n$ given assumptions $\Assumps$ and global environment $\env$. This function is defined as
