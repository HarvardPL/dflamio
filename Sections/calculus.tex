...
Finally, the evaluation on the source node is suspended using an expression $\wait[\type]$, which waits for a value of type $\type$. Second, Rule \ruleref{G-Step-Ret} returns control to a suspended source node when the top of the execution stack on the target node has reduced to a term.
...
Rule~\ruleref{Acts-For} in Figure~\ref{fig:top-level-act-for-judgment} is the top-level judgment for deriving trust, which iterates through the strategy principals in the current strategy and attempts to prove the trust relationship. Rule~\ruleref{Acts-For-Cons-1} states that, once a strategy principal proves the trust, the query succeeds, and \ruleref{Acts-For-Cons-2} states that, if a strategy principal cannot prove the trust relationship ($\notactsforquery{C}[\stratelem \meet \conf{n}]{p}{q}$), the search continues with the tail $\stratelem*$ of the strategy. Finally, if the strategy is $\nil$, no strategy principal could prove the trust relationship, and the proof search fails. In both rules the strategy principal $\stratelem$ is attenuated with the confidentiality of the node ($\conf{n}$) to ensure that the node does not attempt to use delegations above its clearance label.
