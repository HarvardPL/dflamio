...
Finally, the evaluation on the source node is suspended by inserting an expression $\wait[\type]$. Second, Rule \ruleref{G-Step-Ret} returns control to a suspended source node when the top of the execution stack on the target node has reduced to a term.
...
\begin{figure}
    ...
    \and
    \inferrule[Acts-For-Cons-2]{\notactsforquery{\Assumps; n; \env}[\stratelem \meet \conf{n}]{p}{q} \\\\ \actsforquery{\Assumps; n; \env}[\stratelem*]{p}{q}{\level}}{\actsforquery{\Assumps; n; \env}[\cons{\stratelem}{\stratelem*}]{p}{q}{\level}}
    \and
    \inferrule[Acts-For-Nil]{~}{\notactsforquery{C}[\nil]{p}{q}}
    \and
    \inferrule[Acts-For]{\stratelem* = \env(n).\strategy \and \actsforquery{C}[\stratelem*]{p}{q}{\option{\level}}}{\actsforquery{C}{p}{q}{\option{\level}}}
    \end{mathpar}
...
Rule~\ruleref{Acts-For} in Figure~\ref{fig:top-level-act-for-judgment} is the top-level judgment for deriving trust, which iterates through the strategy principals in the current strategy and attempts to prove the trust relationship. Rule~\ruleref{Acts-For-Cons-1} states that, once a strategy principal proves the trust, the query succeeds, and \ruleref{Acts-For-Cons-2} states that, if a strategy principal cannot prove the trust relationship ($\forall \level'.~ \notactsforquery{C}[\!\!\stratelem]{\,p}{q}$), the search continues with the tail $\stratelem*$ of the strategy. Finally, if the strategy is $\nil$, no strategy principal is used, which prevents the use of delegations. Since no delegations are unlabeled, the trust is derived without raising the current label.
